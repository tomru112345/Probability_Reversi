...
if turn:
    # 学習データ(200)(後攻)の勝率
    ai_win = [8176, 1864, 5272, 4762, 3900, 4696, 4092, 3715, 3585, 3594, 3561]
    ai_lose = [1824, 7615, 3881, 4426, 5429,
               4888, 5306, 5732, 5999, 5939, 5951]
else:
    # 学習データ(200)(先攻)の勝率
    ai_win = [0, 928, 1735, 2363, 3061, 3590, 3954, 4074, 4145, 4064, 4248]
    ai_lose = [10000, 8927, 8116, 7414, 6689,
...
for i in range(11):
    ...
    ai_win_late[i] = ai_win[i] / (ai_win[i] + ai_lose[i])
gene_1 = np.array(optimal_win_late)
gene_2 = np.array(random_win_late)
gene_3 = np.array(ai_win_late)

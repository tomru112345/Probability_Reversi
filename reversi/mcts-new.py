...
def next_child_node(node):
    ...
    # アーク評価値の計算
    C_PUCT = 1.0
    t = sum(nodes_to_scores(node.child_nodes))
    pucb_values = []
    for child_node in node.child_nodes:
        pucb_values.append((-child_node.w / child_node.n if child_node.n else 0.0) +
                           C_PUCT * child_node.p * sqrt(t) / (1 + child_node.n))
    # アーク評価値が最大の子ノードを返す
    return node.child_nodes[np.argmax(pucb_values)]
...
def evaluate(node):
    ...
    if node.state.is_done():
        # 勝敗結果で価値を取得
        value = -1 if node.state.is_lose() else 0
    ...
    if not node.child_nodes:
        ...
        policies, value = predict(node.model, node.state)
        ...
        for action, policy in zip(node.state.legal_actions(), policies):
            node.child_nodes.append(
                Node(node.model, node.state.next(action), policy))
        return value
...
class Node:
    ...
    def __init__(self, model, state, p):
        """ノードの初期化"""
